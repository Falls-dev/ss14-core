...
namespace Content.Server.Atmos.EntitySystems
{
    public sealed class FlammableSystem : EntitySystem
    {
        ...
        private void OnCollide(EntityUid uid, FlammableComponent flammable, ref StartCollideEvent args)
        {
            ...
            var (srcUid, srcFlammable, destUid, destFlammable, holdsFireAspectWeapon) = flammable.OnFire
                ? (uid, flammable, otherUid, otherFlammable, theyHold)
                : (otherUid, otherFlammable, uid, flammable, weHold);

            if (holdsFireAspectWeapon)
                return;
            // WD EDIT END
