    ...
    public sealed class ServerBorerSystem : EntitySystem
    {
        ...
        private void OnReproduceAfter(EntityUid uid, BorerHostComponent component, BorerReproduceAfterEvent args)
        {
            if (args.Cancelled || !TryComp(component.Borer, out InfestedBorerComponent? borerComp)
                               ...
                               || !TryComp(uid, out TransformComponent? targetTransform)) return;
            args.Handled = true;
        ...
        {
            if (!TryComp(component.Borer, out InfestedBorerComponent? borerComp)) return;
        ...
        public void ReleaseControl(EntityUid borerUid)
        {
            if (!TryComp(borerUid, out ActionsComponent? wormActComp) ||
                ...
                !borComp.ControlligBrain) return;

            bool wormHasMind = _mindSystem.TryGetMind(borerUid, out var hostMindId, out var hostMind);
            bool bodyHasMind = _mindSystem.TryGetMind((EntityUid) borComp.Host!, out var mindId, out var mind);
            if(!bodyHasMind&&!wormHasMind) return;
            ...
            _action.AddAction(borerUid, ref borComp.ActionBorerReproduceEntity, borComp.ActionBorerReproduce, component: wormActComp);
            _action.AddAction(borerUid, ref borComp.ActionBorerInjectWindowOpenEntity, borComp.ActionBorerInjectWindowOpen, component: wormActComp);
            _action.AddAction(borerUid, ref borComp.ActionBorerScanEntity, borComp.ActionBorerScan, component: wormActComp);
            _action.AddAction(borerUid, ref borComp.ActionBorerBrainTakeEntity, borComp.ActionBorerBrainTake, component: wormActComp);

            _action.RemoveAction(borerUid, borComp.ActionBorerBrainResistEntity, comp: wormActComp);

            _action.RemoveAction((EntityUid)borComp.Host!, borComp.ActionBorerBrainReleaseEntity, comp: bodyActComp);

            _action.RemoveAction((EntityUid)borComp.Host!, borComp.ActionBorerReproduceEntity, comp: bodyActComp);
        ...
        private void OnTakeControlAfter(EntityUid uid, InfestedBorerComponent component, BorerBrainTakeAfterEvent args)
        {
            ...
            if (!TryComp(uid, out ActionsComponent? comp) ||
                ...
                return;
            bool borHasMind = _mindSystem.TryGetMind(uid, out var mindId, out var mind);
            bool hostHasMind = _mindSystem.TryGetMind((EntityUid) component.Host!, out var hostMindId, out var hostMind);
            ...
            _action.RemoveAction(uid, component.ActionBorerOutEntity, comp:comp);
            _action.RemoveAction(uid, component.ActionBorerScanEntity, comp:comp);
            _action.RemoveAction(uid, component.ActionBorerBrainTakeEntity, comp:comp);
            _action.RemoveAction(uid, component.ActionBorerInjectWindowOpenEntity, comp:comp);
            _action.RemoveAction(uid, component.ActionBorerReproduceEntity, comp:comp);
        ...
        private void StartInfest(EntityUid user, EntityUid target, BorerComponent comp)
        {
            _doAfterSystem.TryStartDoAfter(new DoAfterArgs(EntityManager,
                ...
                new BorerInfestDoAfterEvent(), user, target: target)
            {
        ...
        private void OnTelepathicSpeech(EntityUid uid, InfestedBorerComponent component, BorerBrainSpeechActionEvent args)
        {
                ...
                Loc.GetString("borer-ui-converse-message"), (string message) =>
            {
                ...
                _chatManager.ChatMessageToOne(ChatChannel.Local, message, message, EntityUid.Invalid, false, actor.PlayerSession.ConnectedClient);


                if (EntityManager.TryGetComponent(component.Host, out ActorComponent? hostActor))
                {
                    _popup.PopupEntity(message, (EntityUid) component.Host!, (EntityUid) component.Host!, PopupType.Medium);
