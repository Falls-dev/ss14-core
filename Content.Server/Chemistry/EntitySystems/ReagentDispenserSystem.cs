...
public sealed class ReagentDispenserSystem : EntitySystem
{
    ...
    /// <summary>
    /// The next time the game will check if dispensers should be charged
    /// </summary>
    private TimeSpan _nextUpdateTime = TimeSpan.MinValue; // WD
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<ReagentDispenserComponent, ReagentDispenserSetDispenseAmountMessage>(
            OnSetDispenseAmountMessage);

        SubscribeLocalEvent<ReagentDispenserComponent, ReagentDispenserDispenseReagentMessage>(
            OnDispenseReagentMessage);

        SubscribeLocalEvent<ReagentDispenserComponent, ReagentDispenserClearContainerSolutionMessage>(
            OnClearContainerSolutionMessage);
    }
    ...
    {
        if (TryComp<ChemMasterComponent>(args.Sink, out var master) &&
            args.SourcePort == ReagentDispenserComponent.ChemMasterPort)
    ...
    private void UpdateUiState(Entity<ReagentDispenserComponent> reagentDispenser)
    {
        ...
        var state = new ReagentDispenserBoundUserInterfaceState(outputContainerInfo, inventory,
            reagentDispenser.Comp.DispenseAmount, reagentDispenser.Comp.Charge, reagentDispenser.Comp.MaxCharge);
    ...
    private List<ReagentId> GetInventory(Entity<ReagentDispenserComponent> ent)
    {
        ...
        if (reagentDispenser.PackPrototypeId is not null
            && _prototypeManager.TryIndex(reagentDispenser.PackPrototypeId,
                out ReagentDispenserInventoryPrototype? packPrototype))
        {
        ...
        if (HasComp<EmaggedComponent>(ent)
            ...
            && _prototypeManager.TryIndex(reagentDispenser.EmagPackPrototypeId,
                out ReagentDispenserInventoryPrototype? emagPackPrototype))
        {
    ...
    private void OnEmagged(Entity<ReagentDispenserComponent> reagentDispenser, ref GotEmaggedEvent args)
    {
        ...
        // WD EDIT
        reagentDispenser.Comp.MaxCharge = reagentDispenser.Comp.EmaggedMaxCharge;
        reagentDispenser.Comp.RechargeRate = reagentDispenser.Comp.EmaggedRegenRate;
        // WD EDIT END

        args.Handled = true;
    }

    private void OnSetDispenseAmountMessage(
        Entity<ReagentDispenserComponent> reagentDispenser,
        ref ReagentDispenserSetDispenseAmountMessage message)
    {
    ...
    private void OnDispenseReagentMessage(
        Entity<ReagentDispenserComponent> reagentDispenser,
        ref ReagentDispenserDispenseReagentMessage message)
    {
            ...
            !_solutionContainerSystem.TryGetFitsInDispenser(outputContainer.Value, out var solution, out _))
        {
            ...
            reagentDispenser.Comp.Charge -= FixedPoint2.New((int) reagentDispenser.Comp.DispenseAmount);
            bufferSolution.Value.Comp.Solution.AddReagent(message.ReagentId,
                FixedPoint2.New((int) reagentDispenser.Comp.DispenseAmount));
        ...
        if (_solutionContainerSystem.TryAddReagent(solution.Value, message.ReagentId,
                (int) reagentDispenser.Comp.DispenseAmount, out var dispensedAmount)
            && message.Session.AttachedEntity is not null)
        {
            reagentDispenser.Comp.Charge -= FixedPoint2.New((int) reagentDispenser.Comp.DispenseAmount); // WD EDIT
    ...
    private void OnClearContainerSolutionMessage(
        Entity<ReagentDispenserComponent> reagentDispenser,
        ref ReagentDispenserClearContainerSolutionMessage message)
    {
        var outputContainer = _itemSlotsSystem.GetItemOrNull(reagentDispenser, SharedReagentDispenser.OutputSlotName);
        if (outputContainer is not { Valid: true } ||
            !_solutionContainerSystem.TryGetFitsInDispenser(outputContainer.Value, out var solution, out _))
    ...
    public override void Update(float frameTime)
    {
        ...
        if (_nextUpdateTime > curTime)
            return;

        _nextUpdateTime = curTime + _updateFrequency;

        var query = EntityQueryEnumerator<ReagentDispenserComponent, ApcPowerReceiverComponent>();

        while (query.MoveNext(out var uid, out var dispenser, out var powerReceiver))
        {
            ...
            if (dispenser.Charge >= dispenser.MaxCharge)
            {
                continue;
            }

            dispenser.Charge = FixedPoint2.Clamp(dispenser.Charge + dispenser.RechargeRate, 0, dispenser.MaxCharge);
            UpdateUiState(new Entity<ReagentDispenserComponent>(uid, dispenser));
        }
    }
    // WD EDIT END
}
