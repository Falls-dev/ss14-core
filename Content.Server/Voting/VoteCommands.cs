...
namespace Content.Server.Voting
{
    ...
    public sealed class CreateCustomCommand : LocalizedEntityCommands
    {
        ...
        public override void Execute(IConsoleShell shell, string argStr, string[] args)
        {
            ...
            vote.OnFinished += (_, eventArgs) =>
            {
                if (eventArgs.Winner == null)
                {
                    ...
                    DisplayVoteResult(vote, _chatManager, args); // WD
                }
                else
                {
        ...
        private void DisplayVoteResult(IVoteHandle vote, IChatManager _chatManager, string[] args) // WD
        {
            _chatManager.DispatchServerAnnouncement(Loc.GetString("cmd-customvote-on-finished-votes", ("title", vote.Title)));
            for (var x = 1; x < args.Length; x++)
            {
                var option = args[x];
                var votes = vote.VotesPerOption[x];
                _chatManager.DispatchServerAnnouncement(Loc.GetString("cmd-customvote-option-votes", ("option", option), ("votes", votes)));
            }
        }
