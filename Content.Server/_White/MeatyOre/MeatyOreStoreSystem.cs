...
public sealed class MeatyOreStoreSystem : EntitySystem
{
    ...
    private readonly Dictionary<NetUserId, Entity<StoreComponent>> _meatyOreStores = new();
    ...
    private void MeatyOreVerbs(GetVerbsEvent<Verb> ev)
    {
        ...
        if (!TryGetStore(actorComponent.PlayerSession, out var storeEntity))
            return;
        ...
        var verb = new Verb
        {
            ...
            Act = () =>
            {
                TryAddRole(ev.User, ev.Target, storeEntity.Value);
            },
    ...
    private void OnPanelEnableChanged(bool enabled)
    {
        if (!enabled)
        {
            foreach (var storeEntity in _meatyOreStores)
            {
                var session = _playerManager.GetSessionById(storeEntity.Key);
                ...
                _storeSystem.CloseUi(storeEntity.Value);
            }
        }
    ...
    private void OnShopRequested(MeatyOreShopRequestEvent msg, EntitySessionEventArgs args)
    {
        ...
        if (!TryGetStore(playerSession, out var storeEntity))
            return;
    ...
    private bool TryGetStore(ICommonSession session, [NotNullWhen(true)] out Entity<StoreComponent>? storeEntity)
    {
        storeEntity = null!;


        if (!_sponsorsManager.TryGetInfo(session.UserId, out var sponsorInfo)
            || sponsorInfo.MeatyOreCoin == 0)
            return false;

        if (_meatyOreStores.TryGetValue(session.UserId, out var entity))
        {
            storeEntity = entity;
            return true;
        }

        storeEntity = CreateStore(session.UserId, sponsorInfo.MeatyOreCoin);
        return true;
    }

    private void OnPostRoundCleanup(RoundRestartCleanupEvent ev)
    {
        foreach (var entity in _meatyOreStores.Values)
        {
            Del(entity);
        }
    ...
    private Entity<StoreComponent> CreateStore(NetUserId userId, int balance)
    {
        var session = _playerManager.GetSessionById(userId);
        var user = session.AttachedEntity!;

        var storeEntity = Spawn(StorePresetPrototype);
        ...
        _meatyOreStores[userId] = new Entity<StoreComponent>(user.Value, storeComponent);

        return new Entity<StoreComponent>(user.Value, storeComponent);
    }

    private async void TryAddRole(EntityUid user, EntityUid target, Entity<StoreComponent> storeEntity)
    {
        ...
        if (!storeEntity.Comp.Balance.TryGetValue(MeatyOreCurrencyPrototype, out var currency))
            return;
