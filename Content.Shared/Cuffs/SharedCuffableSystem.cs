...
namespace Content.Shared.Cuffs
{
    ...
    public abstract partial class SharedCuffableSystem : EntitySystem
    {
        ...
        {
            component.Container =
                _container.EnsureContainer<Container>(uid, _componentFactory.GetComponentName(component.GetType()));
        }
        ...
        private void OnBeingPulledAttempt(EntityUid uid, CuffableComponent component, BeingPulledAttemptEvent args)
        {
            ...
            if (pullable.Puller != null &&
                !component.CanStillInteract) // If we are being pulled already and cuffed, we can't get pulled again.
                args.Cancel();
        }
        ...
        {
            if (component.CanStillInteract || !EntityManager.TryGetComponent(uid, out PullableComponent? pullable) ||
            ...
            CuffableComponent? cuffable = null)
        {
            ...
            var doAfterEventArgs = new DoAfterArgs(EntityManager, user, cuffTime, new AddCuffDoAfterEvent(), handcuff,
                target, handcuff)
            {
                BreakOnMove = true,
            ...
            HandcuffComponent? cuff = null)
        {
            ...
            else
            {
                ...
                {
                    Log.Warning(
                        "A user is trying to remove handcuffs that aren't in the owner's container. This should never happen!");
                }
            }
            ...
            var doAfterEventArgs = new DoAfterArgs(EntityManager, user, uncuffTime, new UnCuffDoAfterEvent(), target,
                target, cuffsToRemove)
            {
                BreakOnMove = true,
                BreakOnWeightlessMove = false,
                BreakOnDamage = true,
            ...
            _adminLog.Add(LogType.Action, LogImpact.Low,
                $"{ToPrettyString(user)} is trying to uncuff {ToPrettyString(target)}");
            ...
            HandcuffComponent? cuff = null)
        {
            ...
            {
                if (user != null)
                    _popup.PopupPredicted(Loc.GetString("cuffable-component-remove-cuffs-success-message"), user.Value,
                        user.Value);
