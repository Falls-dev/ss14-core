...
public abstract class SharedFluffColorForClothingSystem : EntitySystem
{
    ...
    private void OnAfterHandleState(Entity<FluffColorForClothingComponent> entity, ref AfterAutoHandleStateEvent args)
    {
        UpdateVisuals(entity);
    }

    private void OnInit(Entity<FluffColorForClothingComponent> entity, ref ComponentInit args)
    {
        UpdateVisuals(entity);
    }

    private void OnAddVerb(Entity<FluffColorForClothingComponent> entity, ref GetVerbsEvent<AlternativeVerb> args)
    {
        if (!args.CanAccess || !args.CanInteract || args.Hands == null || entity.Comp.Colors.Count < 2 || !entity.Comp.MainItem)
            return;
    ...
    protected virtual void UpdateVisuals(Entity<FluffColorForClothingComponent> ent)
    {
        // See client system
    }

    protected virtual void ChangeColor(Entity<FluffColorForClothingComponent> ent)
    {
        // See server system
    }
}
